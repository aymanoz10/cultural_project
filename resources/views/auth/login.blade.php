...
<head>
    ...
    <style>
        body {
            ...
            /* 🖼️ دمج صورة الزخرفة مع الظل واللون الزمردي العميق */
            background-image: 
                radial-gradient(circle at 50% 50%, rgba(13, 75, 59, 0.6) 0%, rgba(4, 22, 17, 0.95) 100%),
                url("{{ asset('images/background.jpeg') }}"); /* مسار صورة الزخرفة لديك */
            
            background-repeat: repeat;
            background-size: 1160px; /* التحكم بحجم تكرار الزخرفة */
            background-position: center;
        }

        /* توهج الأنوار الذهبية (Bokeh Lights) */
        .bokeh-top-right {
...
<body class="min-h-screen flex items-center justify-center p-4 relative overflow-hidden antialiased select-none">

    <!-- بقع الضوء الذهبية الداعمة للخلفية -->
    ...
    <!-- بطاقة تسجيل الدخول البيضاء المركزية -->
    <div class="w-full max-w-[400px] bg-white/98 rounded-[32px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] p-8 sm:p-9 relative z-10 text-center border border-white/40 backdrop-blur-sm">

      <!-- الشعار بارز فوق إطار البطاقة -->
<!-- الشعار بارز بنصفه فوق الإطار الأبيض وبنصفه داخل البطاقة -->
<div class="-mt-37 mb-3 flex flex-col items-center justify-center relative z-20">
    <img src="{{ asset('images/logo.png') }}" 
         alt="مديرية الثقافة بدمشق" 
         class="w-56 h-56 object-contain filter drop-shadow-[0_12px_20px_rgba(0,0,0,0.35)]"
         onerror="this.onerror=null; this.src='https://via.placeholder.com/200x180/ffffff/06221a?text=ضع+اللوغو+هنا';">

    <!-- اسم المديرية -->
    <div class="text-[#b58d43] font-black leading-tight mt-1">
        
    </div>
</div>

        <!-- العناوين -->
        <h2 class="text-2xl font-black text-slate-900 mb-1">تسجيل دخول المشرفين</h2>
        <p class="text-[12px] text-slate-500 font-semibold mb-6">أدخل رقم الهاتف وكلمة المرور للوصول إلى لوحة التحكم</p>

        <!-- صندوق عرض التنبيهات والأخطاء -->
        ...
        <form id="login-form" class="space-y-3.5">
            ...
            <div class="relative flex items-center">
                <input type="text" id="phone" name="phone" placeholder="09xxxxxxxx" 
                    class="w-full bg-white border border-slate-200/90 rounded-2xl pr-12 pl-4 py-3 text-sm font-bold text-slate-800 placeholder-slate-300 outline-none focus:border-[#0d4b3b] focus:ring-1 focus:ring-[#0d4b3b] transition" 
            ...
            <div class="relative flex items-center">
                <input type="password" id="password" name="password" placeholder="••••••••" 
                    class="w-full bg-white border border-slate-200/90 rounded-2xl pr-10 pl-10 py-3 text-sm font-bold text-slate-800 placeholder-slate-300 outline-none focus:border-[#0d4b3b] focus:ring-1 focus:ring-[#0d4b3b] transition" 
                    required>
                ...
                <button type="button" id="toggle-password" class="absolute left-3.5 text-slate-400 hover:text-slate-600 focus:outline-none transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- التذييل السفلي -->
        <div class="mt-8 text-[11px] font-bold text-slate-400">
    ...
    <script>
        ...
        document.getElementById('login-form').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                alertBox.classList.remove('hidden');
                alertBox.classList.add('bg-rose-50', 'text-rose-700', 'border', 'border-rose-200');
                alertBox.innerText = 'حدث خطأ أثناء الاتصال بالخادم، يرجى التأكد من الاتصال بالموجّه';
            } finally {
