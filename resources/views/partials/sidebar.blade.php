@php
  ...
  $menuItems = [
    ...
    [
      'route' => 'admin.theaters.index',
      'label' => 'المسارح',
      'roles' => ['super', 'admin'],
      'icon' => 'M7 4v16M17 4v16M3 8h18M3 16h18'
    ],
    [
      'route' => 'admin.libraries.index',
      'label' => 'المكتبات والكتب',
      'roles' => ['super', 'admin'],
    ...
    @foreach ($menuItems as $item)
      @if (in_array($currentRole, $item['roles']))
        @php 
          $isActive = $item['route'] !== '#' && request()->routeIs($item['route'].'*'); 
        @endphp
        <a href="{{ $item['route'] === '#' ? '#' : route($item['route']) }}" onclick="closeMobileSidebar()"
           class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 {{ $isActive ? 'bg-forest dark:bg-[#d4af37] text-white dark:text-[#1a1a1a] shadow-md' : 'text-slate-500 dark:text-gray-300 hover:bg-slate-50' }}">
          <span class="flex items-center justify-center w-8 h-8 rounded-lg {{ $isActive ? 'bg-white/15 dark:bg-[#1a1a1a]/20' : 'bg-slate-100 dark:bg-[#262626]' }}">
            <svg class="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}"/></svg>
          </span>
          {{ $item['label'] }}
        </a>
      @endif
