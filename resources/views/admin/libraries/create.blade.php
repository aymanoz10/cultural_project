...
@section('title', 'إضافة مكتبة جديدة')
@section('page-title', 'إضافة مكتبة جديدة')
...
<div class="max-w-3xl mx-auto">
  <div class="card p-6 md:p-8 space-y-6 bg-white dark:bg-[#181C1A] border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm">
    ...
    <div class="flex items-center justify-between border-b pb-4 border-slate-100 dark:border-white/10">
      <h3 class="text-lg font-black text-slate-900 dark:text-white">بيانات المكتبة الجديدة</h3>
    ...
    <form id="create-library-form" onsubmit="submitCreateLibrary(event)" enctype="multipart/form-data" class="space-y-5">
      @csrf
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="name" placeholder="مثال: مكتبة المركز المركزية" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
        </div>
      </div>

      <!-- صورة المكتبة -->
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">صورة المكتبة (اختياري)</label>
        ...
        <div id="image-preview-wrapper" class="mt-3 hidden">
          ...
          <img id="image-preview" src="#" class="w-24 h-24 object-cover rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
        </div>
      </div>
...
<script>
  ...
  async function submitCreateLibrary(e) {
    e.preventDefault();
    const form = document.getElementById('create-library-form');
    ...
    try {
      const response = await fetch("{{ route('admin.libraries.store', [], false) }}", {
        method: 'POST',
        headers: {
