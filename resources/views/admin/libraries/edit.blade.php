...
@section('page-title', 'تعديل بيانات المكتبة')
...
<div class="max-w-3xl mx-auto">
  <div class="card p-6 md:p-8 space-y-6 bg-white dark:bg-[#181C1A] border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm">
    ...
    <div class="flex items-center justify-between border-b pb-4 border-slate-100 dark:border-white/10">
      <h3 class="text-lg font-black text-slate-900 dark:text-white">تعديل المكتبة: {{ $library->name }}</h3>
      <a href="{{ route('admin.libraries.index') }}" class="text-slate-500 hover:text-slate-700 dark:text-slate-400 text-sm font-bold flex items-center gap-1 transition-colors">
        → العودة للقائمة
      </a>
    </div>
    ...
    <form id="edit-library-form" onsubmit="submitEditLibrary(event)" enctype="multipart/form-data" class="space-y-5">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="cultural_center_id" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
            @foreach($culturalCenters as $center)
              <option value="{{ $center->id }}" {{ $library->cultural_center_id == $center->id ? 'selected' : '' }}>{{ $center->name }}</option>
            @endforeach
          </select>
        </div>
        ...
        <!-- اسم المكتبة -->
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">اسم المكتبة <span class="text-rose-500">*</span></label>
          <input type="text" name="name" value="{{ $library->name }}" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
        </div>
      </div>

      <!-- صورة المكتبة الحالية وتعديلها -->
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">تغيير صورة المكتبة (اختياري)</label>

        <div class="mb-3 flex items-center gap-3">
          @if($library->image)
            <div>
              <span class="block text-[11px] text-slate-400 mb-1">الصورة الحالية:</span>
              <img src="{{ Storage::url($library->image) }}" class="w-20 h-20 object-cover rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
            </div>
          @endif

          <div id="image-preview-wrapper" class="hidden">
            <span class="block text-[11px] text-slate-400 mb-1">الصورة الجديدة:</span>
            <img id="image-preview" src="#" class="w-20 h-20 object-cover rounded-xl border border-amber-500 shadow-sm">
          </div>
        </div>
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-white/10">
        <a href="{{ route('admin.libraries.index') }}" class="px-5 py-2.5 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 font-bold text-sm transition-colors">إلغاء</a>
...
<script>
  ...
  async function submitEditLibrary(e) {
    ...
    try {
      const response = await fetch("{{ route('admin.libraries.update', $library->id, false) }}", {
        method: 'POST',
        headers: {
      ...
      if (response.ok && (res.success || res.data)) {
        window.location.href = "{{ route('admin.libraries.index', [], false) }}";
      } else {
        let errorsList = [];
        if (res.errors) {
