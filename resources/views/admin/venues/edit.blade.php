...
<div class="max-w-3xl mx-auto">
  <div class="card p-6 md:p-8 space-y-6 bg-white dark:bg-[#181C1A] border border-slate-200 dark:border-white/10 rounded-3xl shadow-sm">
    ...
    <div class="flex items-center justify-between border-b pb-4 border-slate-100 dark:border-white/10">
      <h3 class="text-lg font-black text-slate-900 dark:text-white">تعديل القاعة: {{ $venue->name }}</h3>
    ...
    <form id="edit-venue-form" onsubmit="submitEditVenue(event)" enctype="multipart/form-data" class="space-y-5">
      @csrf
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="cultural_center_id" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
            @foreach($culturalCenters as $center)
              <option value="{{ $center->id }}" {{ $venue->cultural_center_id == $center->id ? 'selected' : '' }}>
                {{ $center->name }}
              </option>
            @endforeach
          </select>
        ...
        <div>
          ...
          <input type="text" name="name" value="{{ $venue->name }}" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="venue_type_id" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
            @foreach($venueTypes as $venueType)
              <option value="{{ $venueType->id }}" {{ ($venue->venue_type_id ?? null) == $venueType->id ? 'selected' : '' }}>
                {{ $venueType->name }}
              </option>
            @endforeach
          </select>
        ...
        <!-- السعة الاستيعابية -->
        <div>
          <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">السعة الاستيعابية <span class="text-rose-500">*</span></label>
          <input type="number" name="capacity" min="1" value="{{ $venue->capacity }}" required class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
        </div>
      </div>

      <!-- الميزات -->
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">الميزات (افصل بينها بفواصل)</label>
        <input type="text" id="features-input" value="{{ is_array($venue->features) ? implode(', ', $venue->features) : $venue->features }}" class="form-input w-full bg-slate-50 dark:bg-[#111412] text-slate-900 dark:text-white border border-slate-200 dark:border-white/10 rounded-xl p-3 text-xs focus:ring-2 focus:ring-amber-500 focus:outline-none">
      </div>

      <!-- صورة القاعة الحالية وتعديلها -->
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">تغيير صورة القاعة (اختياري)</label>
        
        <div class="mb-3 flex items-center gap-3">
          @if($venue->image)
            <div>
              <span class="block text-[11px] text-slate-400 mb-1">الصورة الحالية:</span>
              <img src="{{ Storage::url($venue->image) }}" class="w-16 h-16 object-cover rounded-xl border border-slate-200 dark:border-white/10 shadow-sm">
            </div>
          @endif

          <div id="image-preview-wrapper" class="hidden">
            <span class="block text-[11px] text-slate-400 mb-1">الصورة الجديدة:</span>
            <img id="image-preview" src="#" class="w-16 h-16 object-cover rounded-xl border border-amber-500 shadow-sm">
          </div>
        </div>
...
<script>
  ...
  async function submitEditVenue(e) {
    ...
    try {
      const response = await fetch("{{ route('admin.venues.update', $venue->id) }}", {
        method: 'POST',
        headers: { 
