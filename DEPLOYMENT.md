...
## الخطوة 2 — التخزين على Supabase Storage
1. في مشروع **[Supabase](https://supabase.com)** → **Storage** → أنشئ **bucketين**:
   - `cultural-public` — **عام (Public)** — للأغلفة والصور.
   - `cultural-private` — **خاص (Private)** — لملفات PDF (تُقدَّم عبر روابط موقّتة).
2. أنشئ مفاتيح S3: **Storage → S3 access keys → New access key** → احفظ:
   - `AWS_ACCESS_KEY_ID` و `AWS_SECRET_ACCESS_KEY`
   - `AWS_ENDPOINT` = `https://<ref>.storage.supabase.co/storage/v1/s3`
3. رابط الـ bucket العام (`PUBLIC_DISK_URL`):
   `https://<ref>.supabase.co/storage/v1/object/public/cultural-public`

> `<ref>` = معرّف مشروع Supabase. القيم كلها جاهزة في `render.yaml` — تُدخِل الأسرار فقط في لوحة Render.
...
- **الصور لا تظهر:** تأكّد أن `cultural-public` **عام** في Supabase، وأن `PUBLIC_DISK_URL` صحيح، و`FILESYSTEM_DISK=public` و`PUBLIC_DISK_DRIVER=s3`.
