    ...
    */
    ...
    'disks' => [
        ...
        'public' => [
            ...
            // جذر محلي في التطوير، وجذر فارغ (جذر الـ bucket) عند s3 حتى لا يُضاف مسار محلي للمفتاح
            'root'                    => env('PUBLIC_DISK_DRIVER', 'local') === 's3' ? '' : storage_path('app/public'),
            'url'                     => env('PUBLIC_DISK_URL', env('APP_URL').'/storage'),
            'visibility'              => 'public',
            'throw'                   => false,
            // تُستخدم الحقول التالية فقط عندما PUBLIC_DISK_DRIVER=s3 (Supabase / R2 / AWS) —
        ...
        // قرص ملفات الكتب (PDF) الخاص: محلي في التطوير، سحابي خاص في الإنتاج (BOOKS_DISK_DRIVER=s3).
        // يُقدَّم عبر BookFileController الذي يتحوّل تلقائياً إلى روابط موقّتة موقّعة عند s3.
        'books_private' => [
            'driver'                  => env('BOOKS_DISK_DRIVER', 'local'),
            'root'                    => env('BOOKS_DISK_DRIVER', 'local') === 's3' ? '' : storage_path('app/private/books'),
            'visibility'              => 'private',
            'throw'                   => false,
            'key'                     => env('AWS_ACCESS_KEY_ID'),
            'secret'                  => env('AWS_SECRET_ACCESS_KEY'),
            'region'                  => env('AWS_DEFAULT_REGION', 'us-east-1'),
            'bucket'                  => env('AWS_PRIVATE_BUCKET', 'cultural-private'),
            'endpoint'                => env('AWS_ENDPOINT'),
            'use_path_style_endpoint' => true,
        ],
