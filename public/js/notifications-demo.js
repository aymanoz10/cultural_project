...
if (app) {
    ...
    async function fetchApi(suffix, options = {}) {
        ...
        if (!res.ok) {
            throw new Error(data.message || JSON.stringify(data.errors || data) || res.statusText);
        }
        return data;
    }
    ...
    document.addEventListener('click', e => {
        if (dropdown && !dropdown.contains(e.target) && !document.getElementById('bell-btn')?.contains(e.target)) {
